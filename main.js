let input = `Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
...
Game 100: 5 blue, 2 green, 7 red; 14 red, 15 green, 1 blue; 3 blue, 3 red; 8 green, 10 red,`
...
function main() {
  ...
  array = input.split('\n')
  array.forEach(cal => splitter(cal))
  console.log(result)
}
function splitter(input) {
    ...
    if (isImpossible == false) {
      ...
      colors.forEach(color => {
        ...
        if (color.includes("red")) {
          let red = color.match(/\d+/)
          if (parseInt(red[0]) > redCap) {
            isImpossible = true
          }
          // console.log(red)
          // debugger
        ...
        if (color.includes("blue")) {
          let blue = color.match(/\d+/)
          if (parseInt(blue[0]) > blueCap) {
            isImpossible = true
          }
        }

        if (color.includes("green")) {
          let green = color.match(/\d+/)
          if (parseInt(green[0]) > greenCap) {
            isImpossible = true
          }
        }
      })
  ...
  if (isImpossible == true) {
    let addition = game[0].match(/\d+/)
    result += parseInt(addition[0])
    isImpossible = false
  }
}